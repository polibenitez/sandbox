...
MAX_WORKERS = 5  # Número máximo de hilos concurrentes
...
class CityValidator:
    ...
    def _find_latest_partial_file(self, output_path):
        """
        Encuentra el archivo partial más reciente para reanudación.

        Args:
            output_path (str): Ruta del archivo de salida final

        Returns:
            tuple: (ruta_del_archivo_partial, número_del_último_lote) o (None, 0) si no hay partial
        """
        # Obtener el patrón para buscar archivos partial
        file_name, file_ext = os.path.splitext(output_path)
        pattern = f"{file_name}_partial_*{file_ext}"

        partial_files = glob.glob(pattern)

        if not partial_files:
            logger.info("No se encontraron archivos partial para reanudación")
            return None, 0

        # Extraer números de lote y encontrar el más alto
        max_batch = 0
        latest_file = None

        for file_path in partial_files:
            # Extraer número del batch del nombre del archivo
            match = re.search(r"_partial_(\d+)", file_path)
            if match:
        ...
        if latest_file:
            logger.info(
                f"Archivo partial más reciente encontrado: {latest_file} (lote {max_batch})"
            )
            return latest_file, max_batch
        else:
            logger.info("No se pudo determinar el archivo partial más reciente")
            return None, 0

    def _load_partial_dataframe(self, output_path):
        """
        Carga el DataFrame desde el archivo partial más reciente si existe.

        Args:
            output_path (str): Ruta del archivo de salida final

        Returns:
            tuple: (DataFrame o None, último_lote_procesado)
        """
        if not RESUME:
            return None, 0

        latest_partial, last_batch = self._find_latest_partial_file(output_path)

        if latest_partial and os.path.exists(latest_partial):
            try:
                df_partial = self._load_dataframe(latest_partial)

                # Verificar que tiene la columna STANDARDIZED_CITY
                if "STANDARDIZED_CITY" in df_partial.columns:
                    processed_count = len(
                        df_partial[df_partial["STANDARDIZED_CITY"] != "Unknown"]
                    )
                    total_count = len(df_partial)

                    logger.info(f"Cargado archivo partial: {latest_partial}")
                    logger.info(
                        f"Registros procesados: {processed_count}/{total_count}"
                    )

                    return df_partial, last_batch
                else:
                    logger.warning(
                        f"El archivo partial {latest_partial} no tiene columna STANDARDIZED_CITY"
                    )
                    return None, 0

            except Exception as e:
                logger.error(
                    f"Error cargando archivo partial {latest_partial}: {str(e)}"
                )
                return None, 0
...
Your answer:
    ...
    def process_file(self, input_path, output_path, city_column):
        ...
        """
        try:
            # NUEVA LÓGICA: Intentar cargar desde archivo partial
            df_partial, last_batch_processed = self._load_partial_dataframe(output_path)

            if df_partial is not None and RESUME:
                # Usamos el DataFrame partial como base
                df = df_partial.copy()
                logger.info(
                    f"Reanudando desde archivo partial (último lote: {last_batch_processed})"
                )

                # Actualizar el cache con los datos ya procesados del partial
                processed_addresses = df[df["STANDARDIZED_CITY"] != "Unknown"]
                for _, row in processed_addresses.iterrows():
                    if pd.notna(row[city_column]):
                        self.address_to_city_cache[str(row[city_column])] = row[
                            "STANDARDIZED_CITY"
                        ]

                logger.info(
                    f"Cache actualizado con {len(processed_addresses)} direcciones del archivo partial"
                )
            else:
                # Cargar el archivo original
                df = self._load_dataframe(input_path)
                # Inicializar la columna si no existe
                if "STANDARDIZED_CITY" not in df.columns:
                    df["STANDARDIZED_CITY"] = "Unknown"
                logger.info(f"Cargando archivo original: {input_path}")

            # Obtener direcciones únicas que aún necesitan procesamiento
            # Solo considerar las que tienen "Unknown" o NaN en STANDARDIZED_CITY
            mask_needs_processing = (
                (df["STANDARDIZED_CITY"] == "Unknown")
                | (df["STANDARDIZED_CITY"].isna())
                | (df["STANDARDIZED_CITY"] == "")
            ) & (df[city_column].notna())

            addresses_needing_processing = df.loc[
                mask_needs_processing, city_column
            ].unique()

            # Filtrar las que ya están en el cache (protección adicional)
            addresses_to_process = [
                addr
                for addr in addresses_needing_processing
                if str(addr) not in self.address_to_city_cache
            ]

            total_addresses = len(addresses_to_process)
            total_unique = len(df[city_column].dropna().unique())
            already_processed = total_unique - total_addresses

            logger.info(f"Total direcciones únicas en dataset: {total_unique}")
            logger.info(f"Direcciones ya procesadas: {already_processed}")
            logger.info(f"Direcciones pendientes de procesar: {total_addresses}")

            # Si hay direcciones para procesar
            if total_addresses > 0:
                # Calcular desde qué lote continuar
                start_batch_index = (
                    last_batch_processed if RESUME and df_partial is not None else 0
                )
                ...
                with tqdm(
                    ...
                    for i in range(0, total_addresses, self.batch_size):
                        current_batch_number = start_batch_index + i
                        batch = addresses_to_process[i : i + self.batch_size]
                        batch_size = len(batch)

                        logger.info(
                            f"Procesando lote {i // self.batch_size + 1}/{(total_addresses + self.batch_size - 1) // self.batch_size}: {batch_size} direcciones (lote #{current_batch_number})"
                        )
                        ...
                        # Aplicar los nuevos resultados al DataFrame
                        for address in batch:
                            if str(address) in self.address_to_city_cache:
                                city = self.address_to_city_cache[str(address)]
                                mask = (df[city_column] == address) & (
                                    (df["STANDARDIZED_CITY"] == "Unknown")
                                    | (df["STANDARDIZED_CITY"].isna())
                                    | (df["STANDARDIZED_CITY"] == "")
                                )
                                df.loc[mask, "STANDARDIZED_CITY"] = city
                        ...
                        if i + self.batch_size >= total_addresses or (
                            ...
                            temp_output = output_path.replace(
                                ".",
                                f"_partial_{current_batch_number + self.batch_size}.",
                                1,
                            )
                            self._save_dataframe(df, temp_output)
                            logger.info(f"Guardado parcial en {temp_output}")
            else:
                logger.info("No hay nuevas direcciones para procesar")

            # Aplicar mapeo final para asegurarse de que todo esté actualizado
            for address in df[city_column].dropna().unique():
                if str(address) in self.address_to_city_cache:
                    city = self.address_to_city_cache[str(address)]
                    mask = df[city_column] == address
                    df.loc[mask, "STANDARDIZED_CITY"] = city
...
def main():
    ...
    if not RESUME and os.path.exists(output_file_path):
        ...
        # Si se elige sobrescribir, también se eliminan los archivos partial y checkpoint
        if not RESUME:
            # Eliminar archivos partial
            file_name, file_ext = os.path.splitext(output_file_path)
            partial_pattern = f"{file_name}_partial_*{file_ext}"
            partial_files = glob.glob(partial_pattern)

            for partial_file in partial_files:
                try:
                    os.remove(partial_file)
                    logger.info(f"Archivo partial eliminado: {partial_file}")
                except Exception as e:
                    logger.warning(
                        f"Error al eliminar archivo partial {partial_file}: {str(e)}"
                    )
            ...
            if os.path.exists(checkpoint_path):
                ...
                try:
                    if CHECKPOINT_FORMAT == "sqlite":
                        ...
                        logger.info(
                            f"Archivo de checkpoint eliminado: {checkpoint_path}"
                        )
                except Exception as e:
