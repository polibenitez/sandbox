...
class AdvancedTradingAlgorithm:
    ...
    def prepare_features(self, data=None):
        ...
        required_cols = ['sma_20', 'sma_50', 'sma_200', 'rsi', 'macd', 'macd_signal', 
        ...
        for period in [5, 10, 20]:
            ...
            features[f'trend_{period}'] = df['close'].rolling(period).apply(
                lambda x: np.polyfit(np.arange(len(x)), x, 1)[0] / x.iloc[0], 
                raw=True
            )
    ...
    def train_lstm(self, sequence_length=10, epochs=50, batch_size=32):
        ...
        for i in range(len(scaled_features) - sequence_length - 5):
            ...
            future_return = (self.data['close'].iloc[i+sequence_length+5] / 
                             self.data['close'].iloc[i+sequence_length] - 1)
    ...
    def calculate_position_size(self, confidence, volatility, capital=1000.0, max_risk_pct=0.02):
        """
    ...
    def set_risk_management_levels(self, entry_price, signal, atr):
        ...
        take_profit_1 = entry_price + (stop_distance * 1.5) if signal > 0 else entry_price - (stop_distance * 1.5)
        take_profit_2 = entry_price + (stop_distance * 3.0) if signal > 0 else entry_price - (stop_distance * 3.0)
    ...
    def analyze_higher_timeframe(self, data_higher_tf, lookback=20):
        ...
        sma_slope = ((sma_long - recent_data['close'].rolling(window=20).mean().iloc[-5]) / 
    ...
    def advanced_trading_algorithm(self, current_data, higher_tf_data=None):
        ...
        if (current_row['close'] <= current_row['bb_lower'] and 
            current_row['volume'] > current_data['volume'].rolling(20).mean().iloc[-1] * 1.5 and
            current_row['rsi'] < 30):
            signal_bb = 1
        ...
        if (current_data['close'].iloc[-3] > current_data['close'].iloc[-1] and
            current_data['rsi'].iloc[-3] < current_data['rsi'].iloc[-1] and
            current_data['close'].iloc[-1] < current_data['sma_50'].iloc[-1] and
            current_data['rsi'].iloc[-1] < 40):
            signal_rsi_div = 1
        # Divergencia bajista: precio hace máximos más altos pero RSI hace máximos más bajos
        elif (current_data['close'].iloc[-3] < current_data['close'].iloc[-1] and
              current_data['rsi'].iloc[-3] > current_data['rsi'].iloc[-1] and
              current_data['close'].iloc[-1] > current_data['sma_50'].iloc[-1] and
              current_data['rsi'].iloc[-1] > 60):
            signal_rsi_div = -1
        ...
        sentiment = 0
        symbol = current_data.get('symbol', 'BTC')  # Obtener símbolo o usar BTC por defecto
        if isinstance(symbol, pd.Series):
            symbol = symbol.iloc[0] if len(symbol) > 0 else 'BTC'
        
        try:
            sentiment = self.get_sentiment_score(symbol.split('/')[0])
            on_chain = self.get_onchain_metrics(symbol.split('/')[0])
        except Exception as e:
        ...
        if 'sopr' in on_chain:  # Spent Output Profit Ratio para BTC
            ...
            staking_score = (on_chain['staking_rate'] - 0.25) * 4  # Escalar (0.25-0.5 a 0-1)
        ...
        if signal == 1 and higher_tf_trend < -0.5 and current_row['close'] < current_row['sma_200']:
