...
log_filename = f'logs/usage_example_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'
...
def main():
    logger.info("=== INICIANDO APLICACIÓN DE TRADING AVANZADA (EJEMPLO) ===")

    # Configurar la aplicación en modo test
    try:
        logger.info("Inicializando aplicación en modo test para BTC/USDT...")
    ...
    try:
        logger.info("Descargando datos históricos (últimos 200 días)...")
        app.download_historical_data(days_back=200)
        logger.info(f"Datos descargados: {len(app.data)} registros")

        # Mostrar parte de los datos para verificar
        logger.debug(f"Primeros registros: \n{app.data.head()}")
        logger.debug(f"Últimos registros: \n{app.data.tail()}")
    except Exception as e:
    ...
    try:
        logger.info("Aplicando indicadores técnicos...")
        app.apply_indicators()
        logger.info(
            f"Indicadores aplicados. Columnas disponibles: {app.data.columns.tolist()}"
        )
    except Exception as e:
    ...
    try:
        logger.info("Inicializando aplicación para timeframe diario...")
        app_daily = CryptoTradingApp(mode="test", symbol="BTC/USDT", timeframe="1d")
        logger.info("Descargando datos históricos diarios...")
        app_daily.download_historical_data(days_back=200)
        logger.info(f"Datos diarios descargados: {len(app_daily.data)} registros")

        logger.info("Aplicando indicadores técnicos a datos diarios...")
        app_daily.apply_indicators()
        logger.info("Indicadores aplicados a datos diarios")
    except Exception as e:
        logger.error(f"Error al preparar datos de timeframe superior: {e}")
        app_daily = None

    # Inicializar algoritmo avanzado
    try:
        logger.info("Inicializando algoritmo avanzado de trading...")
        advanced_algo = AdvancedTradingAlgorithm(base_data=app.data)
        logger.info("Algoritmo avanzado inicializado correctamente")
    except Exception as e:
        logger.error(f"Error al inicializar algoritmo avanzado: {e}")
        return

    # Entrenar modelos de machine learning (opcional)
    try:
        logger.info("Intentando entrenar modelos de machine learning...")
        ...
        if len(app.data) > 100:
            ...
            logger.info(
                f"Modelo Random Forest entrenado en {time.time() - start_time:.2f} segundos"
            )

            # El entrenamiento LSTM puede ser lento, descomentar si se necesita
            # logger.info("Entrenando modelo LSTM (puede tardar varios minutos)...")
            # start_time = time.time()
            # advanced_algo.train_lstm(epochs=10)  # Menos épocas para ejemplo
            # logger.info(f"Modelo LSTM entrenado en {time.time() - start_time:.2f} segundos")
        else:
            logger.warning(
                f"Datos insuficientes para entrenar modelos ML: {len(app.data)} registros"
            )
    except Exception as e:
        logger.error(f"Error al entrenar modelos: {e}")
        logger.warning("Continuando sin modelos de machine learning")

    # Ejecutar backtesting con el algoritmo avanzado
    logger.info("Iniciando backtesting con algoritmo avanzado...")
    ...
    start_idx = (
        50  # Empezar desde el registro 50 para tener suficientes datos históricos
    )
    end_idx = len(app.data)

    logger.info(f"Ejecutando backtest en {end_idx - start_idx} períodos...")

    # Backtesting
    for i in range(start_idx, end_idx):
        current_date = app.data.index[i]
        logger.debug(f"Procesando fecha: {current_date}")

        # Obtener datos hasta el índice actual (simulando datos disponibles en ese momento)
        current_data = app.data.iloc[: i + 1]
        higher_tf_data = app_daily.data if app_daily is not None else None

        # Aplicar algoritmo avanzado
        try:
            logger.debug(f"Ejecutando algoritmo para {current_date}...")
            trading_decision = advanced_algo.advanced_trading_algorithm(
                current_data=current_data, higher_tf_data=higher_tf_data
            )
            ...
            logger.debug(
                f"Resultado: señal={signal}, precio={current_price:.2f}, confianza={confidence:.4f}"
            )
            ...
            if signal == 1 and position == 0:  # Comprar
                ...
                logger.info(
                    f"COMPRA: {current_time} - Precio: {current_price:.2f}, Confianza: {confidence:.2f}"
                )
                logger.info(
                ...
                logger.info(
                    f"VENTA: {current_time} - Precio: {current_price:.2f}, Confianza: {confidence:.2f}"
                )
                logger.info(f"  Ganancia: {profit_pct:.2f}%")
            ...
            # Verificar stop loss y take profit si tenemos posición abierta
                ...
                if current_price <= stop_loss:
                    ...
                    logger.info(
                        f"STOP LOSS: {current_time} - Precio: {current_price:.2f}"
                    )
                    logger.info(f"  Pérdida: {profit_pct:.2f}%")
                ...
                # Take profit
                    ...
                    logger.info(
                        f"TAKE PROFIT: {current_time} - Precio: {current_price:.2f}"
                    )
                    logger.info(f"  Ganancia: {profit_pct:.2f}%")
            ...
            if (i - start_idx) % 100 == 0 and i > start_idx:
                logger.info(
                    f"Backtest progreso: {((i - start_idx) / (end_idx - start_idx) * 100):.1f}% completado"
                )

        except Exception as e:
            logger.error(f"Error al procesar fecha {current_date}: {e}")
    ...
    if position == 1:
        ...
        logger.info(f"CIERRE FINAL: {last_time} - Precio: {last_price:.2f}")
        logger.info(f"  Ganancia: {profit_pct:.2f}%")

    logger.info("Backtest completado. Analizando resultados...")
    ...
    try:
        ...
        if results_df.empty:
            logger.error("No hay resultados para analizar")
            return
        ...
        logger.info("\n" + "=" * 50)
        logger.info("RESULTADOS DEL BACKTEST (ALGORITMO AVANZADO)")
        logger.info("=" * 50)
        logger.info(f"Balance inicial: 1000.00 USDT")
        logger.info(f"Balance final: {balance:.2f} USDT")
        logger.info(f"Rendimiento total: {total_return:.2f}%")

        # Calcular operaciones
        num_trades = (
            len(signals_df[signals_df["type"] == "buy"]) if not signals_df.empty else 0
        )
        logger.info(f"Número de operaciones: {num_trades}")

        # Calcular operaciones ganadoras/perdedoras
        if not signals_df.empty and "profit_pct" in signals_df.columns:
            winners = len(signals_df[signals_df["profit_pct"] > 0])
            losers = len(signals_df[signals_df["profit_pct"] <= 0])
            logger.info(f"Operaciones ganadoras: {winners}")
            logger.info(f"Operaciones perdedoras: {losers}")

            if winners + losers > 0:
                win_rate = winners / (winners + losers) * 100
                logger.info(f"Porcentaje de éxito: {win_rate:.2f}%")

                # Rendimiento promedio por operación
                avg_win = (
                    signals_df[signals_df["profit_pct"] > 0]["profit_pct"].mean()
                    if winners > 0
                    else 0
                )
                avg_loss = (
                ...
                if avg_loss != 0:
                    ...
                    logger.info(f"Relación riesgo/recompensa: {risk_reward:.2f}")

        logger.info("=" * 50)
    except Exception as e:
        logger.error(f"Error al analizar resultados: {e}")
    ...
    try:
        ...
        plt.subplot(2, 1, 1)
        plt.plot(
            app.data.index[start_idx:],
            app.data["close"].iloc[start_idx:],
        ...
        if not signals_df.empty:
            ...
            if not buys.empty:
                ...
                logger.debug(f"Señales de compra: {len(buys)}")
            ...
            if not sells.empty:
                ...
                logger.debug(f"Señales de venta: {len(sells)}")
            ...
            if not stops.empty:
                ...
                logger.debug(f"Stop loss activados: {len(stops)}")
            ...
            if not takes.empty:
                ...
                logger.debug(f"Take profit activados: {len(takes)}")
        ...
        plt.subplot(2, 1, 2)
        plt.plot(
            results_df["time"], results_df["balance"], label="Balance", color="orange"
        )
        ...
        # Guardar imagen
        chart_filename = f'backtest_advanced_{app.symbol.replace("/", "_")}_{datetime.now().strftime("%Y%m%d")}.png'
        plt.savefig(chart_filename)
        logger.info(f"Gráfico guardado como {chart_filename}")
        ...
        if "composite_score" in results_df.columns:
            ...
            # Guardar imagen
            score_filename = f'composite_score_{app.symbol.replace("/", "_")}_{datetime.now().strftime("%Y%m%d")}.png'
            plt.savefig(score_filename)
            logger.info(f"Gráfico de score compuesto guardado como {score_filename}")
    ...
    try:
        results_csv = f'backtest_results_{app.symbol.replace("/", "_")}_{datetime.now().strftime("%Y%m%d")}.csv'
        results_df.to_csv(results_csv)
        logger.info(f"Resultados guardados en {results_csv}")

        if not signals_df.empty:
            signals_csv = f'backtest_signals_{app.symbol.replace("/", "_")}_{datetime.now().strftime("%Y%m%d")}.csv'
            signals_df.to_csv(signals_csv)
            logger.info(f"Señales guardadas en {signals_csv}")
    except Exception as e:
...
def main():
    ...
    plt.tight_layout()
    plt.savefig(
        f'backtest_advanced_{app.symbol.replace("/", "_")}_{datetime.now().strftime("%Y%m%d")}.png'
    )
    plt.show()
    ...
    if "composite_score" in results_df.columns:
        ...
        plt.tight_layout()
        plt.savefig(
            f'composite_score_{app.symbol.replace("/", "_")}_{datetime.now().strftime("%Y%m%d")}.png'
        )
        plt.show()

    # Guardar los resultados en un CSV para análisis posterior
    results_df.to_csv(
        f'backtest_results_{app.symbol.replace("/", "_")}_{datetime.now().strftime("%Y%m%d")}.csv'
    )
    if not signals_df.empty:
        signals_df.to_csv(
            f'backtest_signals_{app.symbol.replace("/", "_")}_{datetime.now().strftime("%Y%m%d")}.csv'
        )
