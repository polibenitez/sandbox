...
class CryptoTradingApp:
        ...
        if self.mode == "real":
            try:
                ...
                if not api_key or not api_secret:
                    raise ValueError(
                        "API keys no encontradas. Define BINANCE_API_KEY y BINANCE_API_SECRET como variables de entorno."
                    )
    ...
    def trading_algorithm(self, row, prev_row=None):
        ...
        if prev_row is not None:
            ...
            if (
                ...
                and prev_row["sma_20"] <= prev_row["sma_50"]  # Cruce alcista de medias
                and row["rsi"] > 40  # RSI muestra fuerza (pero no sobrecompra)
                and row["macd"] > row["macd_signal"]  # MACD por encima de su señal
                and self.position == 0
            ):  # No tenemos posición abierta
                signal = 1  # Comprar
        ...
        if (
            ...
            and row["volume"]
            > self.data["volume"].rolling(20).mean().iloc[-1]
            * 1.5  # Volumen 50% mayor que la media
            and row["rsi"] < 30  # RSI en sobreventa
            and self.position == 0
        ):
            signal = 1  # Comprar
        ...
        # Venta en resistencia de banda superior
            ...
            and row["rsi"] > 70  # RSI en sobrecompra
            and self.position == 1
        ):
            signal = -1  # Vender
    ...
    def run_backtest(self):
        ...
        for idx, row in self.data.iterrows():
            ...
            if signal == 1 and self.position == 0:  # Comprar
                ...
                amount = (
                    self.balance * 0.95
                )  # Usar 95% del balance (dejando algo para comisiones)
                ...
                logger.info(
                    f"BACKTEST BUY: {idx} - Precio: {price:.2f}, Cantidad: {quantity:.6f}, Total: {amount:.2f}"
                )
                ...
                logger.info(
                    f"BACKTEST SELL: {idx} - Precio: {price:.2f}, Cantidad: {quantity:.6f}, Total: {amount:.2f}"
                )
    ...
    def plot_results(self):
        ...
        plt.subplot(2, 1, 1)
        plt.plot(
            self.data.index, self.data["close"], label="Precio de cierre", alpha=0.7
        )
        ...
        plt.scatter(
            buy_times, buy_prices, color="green", label="Compra", marker="^", s=100
        )
        plt.scatter(
            sell_times, sell_prices, color="red", label="Venta", marker="v", s=100
        )
        ...
        plt.subplot(2, 1, 2)
        plt.plot(
            self.data.index, self.data["rsi"], label="RSI", color="purple", alpha=0.7
        )
        ...
        plt.tight_layout()
        plt.savefig(
            f'backtest_{self.symbol.replace("/", "_")}_{datetime.now().strftime("%Y%m%d")}.png'
        )
        plt.show()
    ...
    def run_live_trading(self, interval_seconds=3600):
        ...
        logger.info(
            f"Iniciando trading en vivo para {self.symbol} con intervalo de {interval_seconds} segundos"
        )
        ...
        try:
            while True:
                ...
                now = datetime.now()
                since = int(
                    (now - timedelta(days=3)).timestamp() * 1000
                )  # Últimos 3 días
                ohlcv = self.exchange.fetch_ohlcv(self.symbol, self.timeframe, since)
                ...
                if signal != 0:
                    ...
                    if result:
                        logger.info(
                            f"Operación ejecutada: {'COMPRA' if signal == 1 else 'VENTA'}"
                        )
                    else:
                ...
                logger.info(
                    f"Esperando {interval_seconds} segundos hasta la próxima comprobación..."
                )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Aplicación de Trading de Criptomonedas"
    )
    parser.add_argument(
        "--mode",
        type=str,
    ...
    parser.add_argument(
        "--days", type=int, default=100, help="Días de datos históricos para backtest"
    )
